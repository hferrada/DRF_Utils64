...
ulong RMQRMM64::queryRMQ(ulong i, ulong j){
	if (i<j && j<nP){
		...
		return rank_1(rmqi(x1,x2))-2;			// this is the child of LCA(x1,x2) in the path from LCA(x1,x2) to x2
	}
	return i;
}
...
/*
ulong RMQRMM64::queryRMQ_beta(ulong i, ulong j){
	if (i<j && j<nP){
		ulong pi = select_1(i+2);
		//ulong pj = select_1(j+2);
		//ulong r = rmqi(pi,pj);
		ulong r = rmqi(pi,select_1(j+2));
		//ulong er = sumAtPos(r);
		//ulong ei = sumAtPos(pi);
		//bool par = readBit64(P,r);

		if(!readBit64(P,r) && sumAtPos(pi)==sumAtPos(r))
			//return rank_1(pi)-2;
			return i;
		else
			return rank_1(r)-2;
